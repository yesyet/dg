...
/////////////////////////////////////////////////
  ...
  async CKLogin() {
    ...
    if (!env) {
      ...
      if (!poolInfo.allowAdd) {
        ...
        this.#sendNotify('用户操作通知', `用户 ${this.nickName}(${decodeURIComponent(this.pt_pin)}) 已上线`);
      }
    } else {
      this.eid = env._id;
      ...
      this.#sendNotify('用户操作通知', `用户 ${this.nickName}(${decodeURIComponent(this.pt_pin)}) 已更新 CK`);
    }
    return {
  ...
  async delUserByEid() {
    ...
    this.#sendNotify('用户操作通知', `用户 ${this.nickName}(${decodeURIComponent(this.pt_pin)}) 删号跑路了`);
    return {
      message: '账户已移除',
    };
  }
...
/////////////////////////////////////////////////
  ...
  async WSCKLogin() {
    ...
    if (!env) {
      ...
      if (!poolInfo.allowWSCKAdd) {
        ...
        this.#sendNotify('用户操作通知', `用户 ${this.pin} WSCK 添加成功`);
      }
    } else {
      this.wseid = env._id;
      ...
      this.timestamp = body.data.timestamp;
      message = `欢迎回来，${this.nickName}`;
      this.#sendNotify('用户操作通知', `用户 ${this.pin} 已更新 WSCK`);
    }
  ...
  async delWSCKUserByEid() {
    ...
    this.#sendNotify('用户操作通知', `用户 ${this.remark}(${decodeURIComponent(this.remark)}) 删号跑路了,CK将无法自动更新并会在不知道那天内自动失效`);
    return {
      message: 'wskey账户已移除',
    };
  }
